...
def get_memcache_key_prefix(key_name)  #取得key_name相关缓存关键字前缀
  ...
  def get_mc(kw_mc, update_cache=false)   #尝试获取相应关键字的memcache信息
    update_cache = false if update_cache.nil?
    kw_mc = kw_mc.gsub(" ","")
    rv = mc(kw_mc)
...
def get_and_save_city(city,ip)
  ...
  def add_remark_mark(user_id)
    REDIS_DB.setex("remark:mark:user:#{user_id}",6.months,1)
  end
  #判断6个月前是否评论过
  def remark_mark_verify(user_id)
   REDIS_DB.get("remark:mark:user:#{user_id}").nil? ? true : false
  end
  #判断验证码是否正确
  def mobile_code_verify(user_id ,code)
    get_code = REDIS_DB.get("mobile_code:#{user_id}")
    if get_code.nil?
      false
    else
