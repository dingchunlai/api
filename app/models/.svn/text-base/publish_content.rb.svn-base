class PublishContent < ActiveRecord::Base
...
#  acts_as_readonlyable [:read_only_51hejia]
  ...
  #专题，专访推广
  def self.search_theme(column_id,page)
    CACHE.fetch("search/theme/#{column_id}/#{page}",1.hours) do
      PublishContent.paginate :page => page,
        :per_page => 26,
        :select => "title,url,created_at,publish_column_id",
        :conditions => ["publish_column_id in (?)",column_id],
